    ...
    $(document).ready(function(){
		...
		var ut_word_rotator = function() {
			...
			var ut_rotator_words = [
				'師資講究教學方法的<span>線上英文</span>',
				'不浪費你生命的<span>線上英文</span>',
				'師資淘汰最嚴的<span>線上英文</span>',
				'師資教學態度最好的<span>線上英文</span>',
				'<span>紐約客線上英文</span>'
			] ,
			counter = 0;                
		...
		$('.btn-holder a[href*=#]').click( function(event) { 
		...
		/* Maginfic POPUP for fee
		================================================== */
		$('.fee-popup').magnificPopup({
			type: 'image',
		...
		/* fee section */
		function animate_fee() {
				
			$('.fee-section .fee-box').css('opacity' , 0).each(function( k ) {
		...
		function animate_sections() {
			...
			$('#portfolio-section').waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					...
					setTimeout( animate_portfolio , 1);
			...
			$('#fee-section').waypoint( function( direction ) {
								
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					
					setTimeout ( function () {
							$('#fee-section header').find('.section-title').animate({ opacity: 1 }).addClass('animated');
					}, 1 );
					
					setTimeout ( function () {
							$('#fee-section header').find('.section-slogan').animate({ opacity: 1 }).addClass('animated');
					}, 1 );
					
					setTimeout( animate_fee , 1);
